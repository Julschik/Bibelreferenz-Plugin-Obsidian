import type { BibleStructure, BookData } from '../types';

/**
 * Complete Bible structure with chapter and verse counts
 * Based on Lutherbibel 2017 verse numbering
 *
 * Format: bookId => { chapters: total, versesPerChapter: [array of verse counts] }
 */
 ...
 * @returns Maximum verse number or null if invalid
 */
export function getMaxVerse(bookId: string, chapter: number): number | null {
  const book = BIBLE_STRUCTURE[bookId];
 ...
 * @param bookId Canonical book ID
 * @returns Maximum chapter number or null if invalid
 */
 ...
 * @param bookId Canonical book ID
 * @param chapter Chapter number (1-indexed)
 * @param verse Optional verse number (1-indexed)
...
/**
 * Canonical order of Bible books (for sorting)
 */
